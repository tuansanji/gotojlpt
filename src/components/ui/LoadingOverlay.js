...
/**
 * Component hiển thị màn hình tải (Loading Overlay) với hiệu ứng đồng hồ cát SVG.
 * Component này che phủ toàn bộ màn hình với nền mờ.
 */
const LoadingOverlay = () => {
  // CSS cho SVG Loader (Tích hợp ngay trong component)
  const loaderStyles = `
    /* CSS cho SVG Loader */
    .loader {
        --dur: 2s;
        display: block;
        margin: auto;
        width: 80px; /* Kích thước đồng hồ cát ĐÃ TĂNG */
        height: 80px; /* Kích thước đồng hồ cát ĐÃ TĂNG */
        --hue: 190; /* Sử dụng màu xanh dương cho đồng hồ cát */
    }
    .loader__glare-top,
    ...
    @keyframes motion-thick {
        ...
        20% { animation-timing-function: cubic-bezier(0.33, 1, 0.67, 1); stroke: hsl(var(--hue), 90%, 20%); stroke-dashoffset: 141.11; transform: rotate(1turn); }
    ...
    @keyframes motion-thin {
        ...
        24% { animation-timing-function: cubic-bezier(0.33, 1, 0.67, 1); stroke: hsl(var(--hue), 90%, 30%); stroke-dashoffset: 134.7; transform: rotate(0.67turn); }
  ...
  return (
    // Đã đổi bg-black/20 thành bg-black/10 để tăng độ trong suốt
    <div className="fixed inset-0 z-50 flex flex-col items-center justify-center bg-black/10 backdrop-blur-sm">
      ...
      <p className="mt-4 mb-10 text-2xl font-semibold text-white md:text-4xl animate-pulse">
        Đang tải ...
      </p>
    </div>
  );
